...
% ============================================================
\begin{abstract}
We introduce SparseWave, a method for encoding sequential data as sparse frequency-domain waveforms via learned amplitude--phase representations across spectral bins. A neural encoder maps input sequences to frequency coefficients, which are transformed via inverse FFT into time-domain waveforms. Energy minimization during training drives the encoder to concentrate information into a small number of active frequency bins. Trained on byte-level text across seven scientific domains, SparseWave achieves 86.1\% byte-level reconstruction fidelity and 99.4\% next-waveform prediction accuracy while activating only 6 of 64 frequency bins (90.6\% spectral sparsity). A controlled spectral sparsity experiment confirms this finding: downstream language models trained on only the top-8 frequency bins achieve perplexity within 2\% of models using all 64 bins (PPL 23.6 vs.\ 23.1), and within 4.4\% of a dense learned-embedding baseline (PPL 22.6). These results demonstrate that language information, when projected into the frequency domain, is naturally sparse---most spectral energy is redundant. If this sparsity transfers to large-scale architectures, it implies that the effective dimensionality of linguistic representations is far smaller than current dense models assume, with direct implications for computational efficiency.
\end{abstract}
...
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item Empirical evidence that byte-level text, when projected into the frequency domain, exhibits natural spectral sparsity: 90.6\% of frequency bins converge to noise floor.
    \item A controlled experiment demonstrating that downstream language modeling performance degrades by less than 2\% when 87.5\% of the spectrum is discarded.
    \item Cross-domain validation across seven scientific disciplines showing that spectral encoding captures structural properties of language rather than domain-specific features.
\end{enumerate}
...
For illustrative purposes, \Cref{tab:flops_projections} extrapolates the measured 90.6\% sparsity ratio to industry-standard architectures.

\begin{table}[t]
\centering
\caption{Projected per-layer FLOPS reduction when applying SparseWave's measured 90.6\% sparsity to standard architectures. These are extrapolations; large-scale validation has not been performed.}
...
\midrule
GPT-4 ($\sim$1.8T)     & 12,288 & $1.81 \times 10^9$ & 90.6\% \\
Frontier 175B           & 8,192  & $8.05 \times 10^8$ & 90.6\% \\
Llama 3 70B             & 8,192  & $8.05 \times 10^8$ & 90.6\% \\
GPT-3.5 ($\sim$20B)     & 4,096  & $2.01 \times 10^8$ & 90.6\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Important:} These projections assume that spectral sparsity observed at small scale transfers to large-scale models, and that hardware can exploit unstructured sparsity efficiently. Both assumptions require validation (see \Cref{sec:limitations}).
...
\paragraph{Scale.} All experiments are conducted at small scale: 64 frequency bins, 128-byte chunks, and a corpus of $\sim$20 million bytes. The key open question is whether spectral sparsity persists at the scale of modern language models ($d_\text{model}$ in the thousands, corpora in the trillions of tokens). We have not trained a SparseWave encoder at scale, and it is possible that larger vocabularies, longer contexts, or more complex language phenomena require denser spectral representations.
...
\paragraph{Reconstruction fidelity.} At 86.1\% mean byte-level reconstruction, SparseWave is not lossless. The 13.9\% reconstruction error is non-trivial and means that the waveform representation discards some information. While this does not affect the spectral sparsity finding (the downstream experiment measures language modeling quality, not reconstruction), it means SparseWave is not directly usable as a compression scheme in its current form.
...
\paragraph{Not all operations benefit.} Even in a sparse representation, softmax in attention, layer normalization, residual connections, and embedding lookups remain dense operations. The 90.6\% reduction applies only to dimension-dependent matrix multiplications (attention projections and feed-forward layers), not to the full forward pass.

\paragraph{Corpus limitations.} The training corpus consists exclusively of public-domain scientific texts in English. Performance on other languages, domains (e.g., code, dialogue, creative writing), or text formats has not been evaluated. The spectral sparsity finding may not generalize to all forms of text.
...
SparseWave demonstrates that byte-level text, when encoded in the frequency domain by a learned neural encoder, produces naturally sparse spectral representations. Without any explicit sparsity penalty, 90.6\% of frequency bins converge to near-zero amplitude. A controlled downstream experiment confirms that this sparsity is not mere lossy compression: language models trained on only the top-8 frequency bins achieve perplexity within 2\% of models using all 64 bins.

The central insight is that \emph{language information is spectrally sparse}. Most of the frequency bandwidth in a spectral representation of text is unused---the information concentrates in a small number of spectral peaks. If this property holds at larger scales, it implies that current dense language model representations are significantly over-provisioned, and that architectures operating on sparse spectral features could achieve comparable performance at a fraction of the computational cost.

We release these findings to encourage further investigation into frequency-domain representations for language, spectral sparsity at scale, and the design of architectures that exploit representation-level sparsity rather than attention-level or parameter-level sparsity alone.
